        ...
        <Masonry gutter="3rem">
          {items.map((item) => (
            <motion.div
              ...
              className="relative group cursor-pointer bg-black-700/20 border border-[#333] hover:border-[var(--accent)] transition-all duration-300 bg-white/5 flex flex-col"
              whileHover={{ y: -5 }}
            >
              {/* Image Area */}
              ...
              <div className="p-6 flex flex-col gap-4">
                ...
                {/* Description */}
                <p className=" publicSans-text text-[#888] text-sm leading-relaxed line-clamp-3">
                  {item.description}
                </p>
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {item.skills.split("•").map((skill, idx) => (
                    <span
                      key={idx}
                      className="border border-[#333] px-3 py-1.5 text-[10px] publicSans-text font-bold uppercase tracking-wider text-white hover:text-white hover:border-white transition-colors"
                    >
                      {skill.trim()}
                    </span>
                  ))}
